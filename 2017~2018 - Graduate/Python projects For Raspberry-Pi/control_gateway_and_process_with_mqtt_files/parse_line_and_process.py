...
def parse_line_and_process(mqtt_line):
	...
	file_printer.print_time_and_eui_fCnt(dev_eui, fCnt)
	...
	if fCnt_is_same:
		...
		print ("hex_received_data_header: " + str(hex_received_data_header) )
		...
		if hex_received_data_header < 0xFC:
		...
		if hex_received_data == None:
			#print("parse_line_and_process 55")
			return None
		else:
			meter_type = dictionary['fPort']
			#print("parse_line_and_process 59")
...
def check_if_fCnt_is_same_previous(dev_eui, fCnt):
	global previous_fCnt_dictionary
...
def process_hex_data( dev_eui, meter_type, hex_data ):
	...
	index_of_this = meter_type - 2
	serial_20byte = various_serial_20bytes[ index_of_this ]
	#serial_20byte = "test" + dev_eui
