...
LorawanError_t LORAWAN_RxDone (uint8_t *buffer, uint8_t bufferLength)
{
	...
	ptr += sprintf(ptr, "bufferLength: %d\r\nbuffer:", bufferLength);
	...
	*/
	...
	//UART_Put_Line ("LORAWAN_RxDone");

    if (loRa.macStatus.macPause == DISABLED)
    {
		//UART_Put_Line ("macPause == DISABLED: lorawan.c 1137");
        mhdr.value = buffer[0];
        if ( (mhdr.bits.mType == FRAME_TYPE_JOIN_ACCEPT) && (loRa.activationParameters.activationType == 0) )
        {
            ...
            {
				//UART_Put_Line ("MIC ERROR - lorawan.c:1160");				
        ...
        {
			//UART_Put_Line ("normal downlink message: lorawan.c 1198");
            ...
            {
				//UART_Put_Line ("devAddr Error: lorawan.c 1323");
            ...
            if (computedMic != extractedMic)
            {
				//UART_Put_Line ("MIC Error: lorawan.c 1344");
                SetReceptionNotOkState();
                if (CLASS_C == loRa.deviceClass)
                {
                    loRa.macStatus.macState = CLASS_C_RX2_2_OPEN;
            ...
            if (hdr->members.fCnt >= loRa.fCntDown.members.valueLow)
            {
                ...
                {
					//UART_Put_Line ("fCnt error - lorawan.c:1359");
            ...
            {                
                ...
                {
					//UART_Put_Line ("fCnt error - lorawan.c:1393");
                    SetReceptionNotOkState();
                    if (CLASS_C == loRa.deviceClass)
                    {
                        loRa.macStatus.macState = CLASS_C_RX2_2_OPEN;
            ...
            {
				uint8_t *ptr = buffer;
				//UART_Put_Line ("lorawan.c 1443: if (hdr->members.fCtrl.fOptsLen != 0)");				
				
				//UART_Put_Line ("No MacCommandsExcute");
                buffer = MacExecuteCommands(hdr->members.MacCommands, hdr->members.fCtrl.fOptsLen);
				//buffer = MacExecuteCommands(buffer, hdr->members.fCtrl.fOptsLen);
				//return_value_of_MacExecuteCommands = MacExecuteCommands(hdr->members.MacCommands, hdr->members.fCtrl.fOptsLen);
				//buffer += 5;
				//buffer += hdr->members.fCtrl.fOptsLen;
				//sprintf(test, "difference_of_MacExecuteCommands: %d, fOptsLen: %d",
				//		buffer - ptr, hdr->members.fCtrl.fOptsLen);
				//UART_Put_Line(test);
            }
			else
			{			
            ...
            {
                fPort = *(buffer++);				
				//sprintf(test, "fPort: %d, fCntDown: %d", fPort, hdr->members.fCnt);
				//UART_Put_Line(test);
                ...
                if (fPort != 0)
                {
					//UART_Put_Line ("lorawan.c 1454: EncryptFRMPayload");
                ...
                {
					//UART_Put_Line ("lorawan.c 1459: fPort == 0");
        ...
        {
			//UART_Put_Line ("the mType is incorrect - lorawan.c:1541");
...
static uint8_t* MacExecuteCommands (uint8_t *buffer, uint8_t fOptsLen)
{
	uint8_t return_value_of_LINK_ADR_CID, test[50];
    bool done = false;
    uint8_t *ptr;
    ptr = buffer;
    while ( (ptr < ( buffer + fOptsLen )) && (done == false) )
    {
        ...
        switch (*ptr ++)
        {
            ...
            case LINK_ADR_CID:
            {
				//UART_Put_Line("LINK_ADR_CID");
                ptr = ExecuteLinkAdr (ptr );
				//sprintf(test, "increment_of_LINK_ADR_CID: %d", ptr - buffer);
				//UART_Put_Line(test);
            } break;

            case DUTY_CYCLE_CID:
            {
				//UART_Put_Line("DUTY_CYCLE_CID");
                ptr = ExecuteDutyCycle(ptr);
            } break;

            case RX2_SETUP_CID:
            {
				//UART_Put_Line("RX2_SETUP_CID");
                ptr = ExecuteRxParamSetupReq (ptr);
            } break;

            case DEV_STATUS_CID:
            {
				//UART_Put_Line("DEV_STATUS_CID");
                ptr = ExecuteDevStatus (ptr);
            } break;

            case NEW_CHANNEL_CID:
            {
				//UART_Put_Line("NEW_CHANNEL_CID");
                ptr = ExecuteNewChannel (ptr);
            } break;

            case RX_TIMING_SETUP_CID:
            {
				//UART_Put_Line("RX_TIMING_SETUP_CID");
                ptr = ExecuteRxTimingSetup (ptr);
            } break;

            default:
            {
				//UART_Put_Line("Unknown MAC commands");
		...
		//ptr = ExecuteLinkAdr (ptr );
		//sprintf(test, "1691: increment_of_ptr: %d", ptr - buffer);
		//UART_Put_Line(test);
	...
	//sprintf(test, "1701: increment_of_ptr: %d", ptr - buffer);
	//UART_Put_Line(test);
