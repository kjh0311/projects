                                                   ...
                                                   date)
{
  ...
  most_nouns <- names(head(sorted_words_count, 20))
...
SNA <- function(sorted_words_count)
{
  ...
  cat(sorted_words, file = "./SNS/candidate.txt", sep = "\n")
  
  row <- paste(sorted_words, sorted_words_count, sep=",")
  cat(row, file = "./SNS/candidate_table.csv", sep = "\n")
}
...
wordcloud <-function(words)
{
  ...
  library(wordcloud)
  wordcloud::wordcloud(names(wordcount), freq = wordcount, scale = c(8,0.5), rot.per = 0, min.freq = 2500, random.order = F, random.color = T, colors = palete)
}
