# 특정 월에 대해서만 워드 클라우드 하기

month <- "08"
year <- "2012"


output_folder_name <- "SNS/2012/02"
output_file_name_for_sna <- paste(output_folder_name, ".txt", sep="")

pick_month_data <- function(data, str_month, str_year)
{
...
get_input_file_name <- function()
{
  file_name <- "Data_OnlySNS2012_2016.txt"
  return(file_name)
}


#get_output_folder_name <- function()
#{
#  return("SNS 2015.8")
#}
...
preprocess_contents <- function(contents)
{
  ...
  contents <- gsub("소셜\\s*네트워크\\s*서비스", "소셜네트워크서비스", contents, perl=TRUE)
...
data<- pick_month_data(data, month, year)
                                                   ...
                                                   date)
{
  ...
  most_nouns <- names(head(sorted_words_count, 10))
...
wordcloud <-function(words, data_length)
{
  ...
  wordcount[line_delimeter_word] <- 0
  wordcount["SNS"] <- 0
  ...
  library(wordcloud)
  wordcloud::wordcloud(names(wordcount), freq = wordcount, scale = c(8,0.5), rot.per = 0, min.freq = min_freq, random.order = F, random.color = T, colors = palete)
}

wordcloud(words, length(data$V1))
...
SNA <- function(sorted_words_count)
{
  ...
  #sorted_words_count <- Filter(function(x) {x >= 3}, sorted_words_count)   # 두 글자 이상 단어만 저장
  ...
  write.table(data, file=output_file_name_for_sna, row.names=FALSE, col.names=FALSE, sep="\t")
}
