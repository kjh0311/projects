...
sns <- 3
using_data <- social_network_service
...
words_count[[line_delimeter_word]]<-NULL
...
SNA <- function(sorted_words_count)
{
  ...
  cat(sorted_words, file = "./SocialNetworkService/candidate.txt", sep = "\n")
  
  row <- paste(sorted_words, sorted_words_count, sep=",")
  cat(row, file = "./SocialNetworkService/candidate_table.csv", sep = "\n")
}
...
wordcloud <-function(words)
{
  ...
  wordcount["SNS"] <- wordcount["SNS"] * 2 / 3
  wordcount["소셜네트워크서비스"] <- wordcount["소셜네트워크서비스"] * 3 / 4
  ...
  library(wordcloud)
  wordcloud::wordcloud(names(wordcount), freq = wordcount, scale = c(6,0.5), rot.per = 0, min.freq = 10, random.order = F, random.color = T, colors = palete)
}
